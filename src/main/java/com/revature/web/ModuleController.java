...
public class ModuleController {
	...
	/*
	 * @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE) public List<Tag>
	 * findAllTags(Tag tag) { return ModuleService.findAllTags(); }
	 */
	...
	public ResponseEntity<?> findModuleByName(@PathVariable("ModuleName") String moduleName) {
		try {
			Module module = ModuleService.findByModuleName(moduleName);
			return ResponseEntity.ok(module);
		} catch (RuntimeException e) {
			return (ResponseEntity<?>) ResponseEntity.badRequest();
		}
